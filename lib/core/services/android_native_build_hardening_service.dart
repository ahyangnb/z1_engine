...
class AndroidNativeBuildHardeningResult {
  const AndroidNativeBuildHardeningResult({required this.logs});

  final List<String> logs;
}

class AndroidNativeBuildHardeningService {
  Future<AndroidNativeBuildHardeningResult> apply({
    required String projectPath,
    bool enableCfi = false,
    bool enableHiddenVisibility = false,
  }) async {
    final logs = <String>[];
    final androidDirectory = _resolveAndroidDirectory(projectPath);
    if (androidDirectory == null) {
      return AndroidNativeBuildHardeningResult(
    ...
    await _writeHardeningFiles(
      androidDirectory,
      logs,
      enableCfi: enableCfi,
      enableHiddenVisibility: enableHiddenVisibility,
    );
    await _ensureGradleApply(appBuildFile, logs);
    await _stripPrebuiltSharedLibraries(androidDirectory, logs);
    logs.add('SO 构建加固配置完成。');

    return AndroidNativeBuildHardeningResult(logs: logs);
  }
  ...
  Future<void> _writeHardeningFiles(
    ...
    List<String> logs, {
    required bool enableCfi,
    required bool enableHiddenVisibility,
  }) async {
    ...
    await _writeFileIfChanged(
      File(_joinPath(nativeDirectory.path, 'z1_native_hardening.properties')),
      [
        'z1.enableCfi=$enableCfi',
        'z1.enableHiddenVisibility=$enableHiddenVisibility',
        '',
      ].join('\n'),
      logs,
      label: 'SO 加固选项',
...
def z1NativeCppFlags = z1NativeCFlags.toList()
...
def z1HardeningProperties = new Properties()
def z1HardeningPropertiesFile = rootProject.file("z1_native/z1_native_hardening.properties")
if (z1HardeningPropertiesFile.exists()) {
    z1HardeningPropertiesFile.withInputStream {
        z1HardeningProperties.load(it)
    }
}
def z1Option = { name, defaultValue ->
    def gradleValue = providers.gradleProperty(name).orNull
    def environmentName = name.replace(".", "_").toUpperCase()
    def environmentValue = System.getenv(environmentName)
    return (gradleValue ?: environmentValue ?: z1HardeningProperties.getProperty(name, defaultValue))
            .toBoolean()
}
def z1EnableCfi = z1Option("z1.enableCfi", "false")
def z1EnableHiddenVisibility = z1Option("z1.enableHiddenVisibility", "false")

if (z1EnableHiddenVisibility) {
    z1NativeCFlags += ["-fvisibility=hidden"]
    z1NativeCppFlags += ["-fvisibility=hidden", "-fvisibility-inlines-hidden"]
}
        ...
        externalNativeBuild {
            cmake {
                ...
                arguments "-DZ1_ENABLE_NATIVE_HARDENING=ON",
                        ...
                        "-DZ1_ENABLE_HIDDEN_VISIBILITY=${z1EnableHiddenVisibility ? "ON" : "OFF"}",
...
function(z1_enable_native_hardening target_name)
    ...
    if (Z1_ENABLE_HIDDEN_VISIBILITY)
        target_compile_options(${target_name} PRIVATE
                -fvisibility=hidden
